<template>
	<div>
		<van-nav-bar fixed safe-area-inset-top class="nav_bg" @click-right="openSearchPage"
			@click-left="showMenu(true)">
			<template #right>
				<van-icon name="search" color="#fff" size="30" />
			</template>
			<template #left>
				<van-icon name="bars" color="#fff" size="30"></van-icon>
		...
		<van-tabbar fixed safe-area-inset-bottom v-model="active" active-color="#00aaff" inactive-color="#707070"
			@change="onChange">
	...
	export default {
		...
		methods: {
			...
			showMenu(show) {
				// this.clicked = !show;
				this.show = show
				console.log("showMenu :" + this.show)
			},
			openSearchPage() {
				console.log("openSearchPage")
			}
		},
	}
</script>
